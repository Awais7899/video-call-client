...
export default function AuthHeader({title,to}) {
    return (
        <Grid container>  
        <Box sx={{ flexGrow: 1 }}>
            <AppBar position="fixed" elevation={0} style={{borderBottom:"1px solid #C5FBC5", background: '#FFFFFF', height: '70px' }}>
                <Grid container>
                    <Grid xs={2} />
                    ...
                    <Grid xs={4} style={{ textAlign: 'right', paddingTop: '24px' }}>
                        <Link style={{ color: '#02B100',border:"none" }} to={to}>
                           {title}
                        </Link>
                    </Grid>
                    <Grid xs={2} />
